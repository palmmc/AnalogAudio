...
public class CassetteDeckBlockRenderer implements BlockEntityRenderer<CassetteDeckBlockEntity> {
    ...
    public CassetteDeckBlockRenderer(BlockEntityRendererProvider.Context context) {
        this.font = context.getFont();
    }

    @Override
    public void render(CassetteDeckBlockEntity entity, float partialTick, PoseStack poseStack, MultiBufferSource buffer,
    ...
    private void renderCassette(CassetteDeckBlockEntity entity, ItemStack cassette, PoseStack poseStack,
            MultiBufferSource buffer, int combinedLight, int combinedOverlay, float alpha, float animProgress) {
        poseStack.pushPose();

        Direction facing = entity.getBlockState().getValue(CassetteDeckBlock.FACING);
